...
def countriesListpage(request, page):
    context = {"results": COUNTRIES[(page-1)*10:page*10], "alfabet": alfabet(), "pagination": pagination()}
    return render(request, "countrieslist.html", context)

def countryblank(request, country_name):
    for el in COUNTRIES:
        if el['country'] == country_name:
            context = {"results": el['languages'], "title":country_name}

    return render(request, "countryblank.html", context)
...
def countryForThisLanguage(request, language):
    lsts = []
    for el in COUNTRIES:
        for el1 in el['languages']:
            if el1 == language:
    ...
    context = {"results": lsts, "title":language}
    return render(request, "countryForThisLanguage.html", context)
...
def _load_main():
    _load_country_to_db_temp()
    _load_language_to_db_temp()
    _load_world_to_db_temp()

def _load_country_to_db_temp():
    for el in COUNTRIES:
        Item = Countries(country_name = el['country'])
        Item.save()

def _load_language_to_db_temp():
    for el in COUNTRIES:
        for el1 in el['languages']:
            Item = Languages(language_name = el1)
            Item.save()

def _load_world_to_db_temp():
    for el in COUNTRIES:
        for el1 in el['languages']:
